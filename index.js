...
function initializeGlide() {
  ...
  if (glide) {
    ...
    // Retry after a short delay if `.glide` is not yet available
    setTimeout(initializeGlide, 100);
  }
}

// Call the function to initialize Glide
initializeGlide();
...
// Typeit
function initTypeIt() {
  // Check if #type1 and #type2 are present in the DOM
  if (document.querySelector("#type1") && document.querySelector("#type2")) {
    new TypeIt("#type1", {
      speed: 120,
      loop: true,
    ...
    // Retry after a short delay if elements aren't yet loaded
    setTimeout(initTypeIt, 100);
  }
}

// Call initTypeIt to start the TypeIt animations
initTypeIt();

// JavaScript to toggle the "Read More" section
document.addEventListener("DOMContentLoaded", function() {
  const readMoreBtn = document.getElementById("readMoreBtn");
  const readMoreContent = document.getElementById("readMoreContent");

  if (readMoreBtn && readMoreContent) { // Ensure both elements are found
    ...
    console.error("Element(s) not found: Ensure 'readMoreBtn' and 'readMoreContent' exist in the HTML.");
  }
});
